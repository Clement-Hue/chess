...
TEST(BoardGameTest, should_simulate_a_game)
{
	...
	EXPECT_TRUE(pawn_b.get_legal_square(21)) << "black pseudo legal square invalid";
	EXPECT_FALSE(pawn_w.get_legal_square(44)) << "white legal square invalid";
	pawn_w.move(board[35]);
	EXPECT_EQ(*pawn_w.get_square(), board[35]) << "white hasn't moved";
	EXPECT_FALSE(pawn_b.get_legal_square(21)) << "black legal square hasn't been computed";
...
TEST(BoardGameTest, should_clear_legal_square_between_move)
{
	...
	EXPECT_FALSE(pawn_w.get_legal_square(40));
}
